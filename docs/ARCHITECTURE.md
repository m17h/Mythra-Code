...
| LM Studio API token (optional) | Native host | Windows Credential Manager |
...
LM Studio is configured as a custom App Server provider using the internal ID `openkiwi-lmstudio`, `wire_api = "responses"`, and the user-configured server URL (default `http://127.0.0.1:1234/v1`). The distinct internal ID avoids overriding Codex's reserved built-in `lmstudio` provider while the UI continues to identify these threads as LM Studio. The native host queries LM Studio's richer `/api/v1/models` catalog first so the UI can exclude embedding models and retain context-window, reasoning, and tool-use metadata; older servers fall back to the OpenAI-compatible `/v1/models` endpoint.

An optional API token is stored in Windows Credential Manager and passed only to the local App Server child as `LMSTUDIO_API_KEY`; unauthenticated localhost servers receive LM Studio's conventional placeholder token. Connected-app tools are disabled for this provider, while OpenKiwi's shell, files, permissions, MCP servers, workflows, schedules, and managed sub-agent bridge remain available. The catalog's reported context window is injected into root, resumed, forked, automated, and sub-agent threads.
