...
## Background workers, progress-message guard and failure diagnostics — 2026-09-13 (evening)

Three live Luna/High/Fast reproductions of the installed 1.17.3 Codex path against Cozy Island (terminal environment, a Finder-style minimal environment, and the opt-in Rust live test) all returned `./.venv/bin/python main.py` in 44–48 seconds with no sandbox denials, so the worker itself was not the failure. The earlier "python3 main.py" runs in the audit log came from the superseded 1.17.2 metadata-only design.

The remaining gap was the UI lifecycle: the discovery hook cancelled its worker whenever the project control unmounted, which happens on any project switch or when opening Chats while waiting, and nothing reported it. Workers now live in a module store keyed by project folder. Switching projects no longer cancels a discovery; the result saves to the project that started it through the save callback captured at start, and the control shows the in-flight or saved state again when that project is reopened. Stop remains explicit and still suppresses late saves.

Both live runs also showed Codex narrating progress in schema-shaped messages whose `command` field was prose. The native prompt now forbids progress messages and demands a single final JSON reply, and validation rejects the observed first-person/status narration and explicit project-observation phrases while preserving English-named executables and shell builtins. Because ephemeral native sessions leave no transcript, a failed native discovery now writes sanitized provider/model/effort/cwd/status metadata, output byte counts and a fixed failure category to `run-discovery-last-failure.log` in the app data folder. No provider stdout, stderr or error text is persisted, including source lines and secret-bearing assignments; the transient popover error still prefers the last safe error-like stderr line over echoed tool output.

Follow-up: the progress-message guard now matches narration phrases instead of rejecting commands based on their first word or sentence-like punctuation. Regression cases accept `next dev`, `next start`, other English-named launchers and shell builtins while continuing to reject the observed progress messages, including “Next, I’ll…” narration. Focused native/UI tests and Clippy validate this correction.
