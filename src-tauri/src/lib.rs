...
async fn spawn_server(app: &AppHandle) -> Result<Arc<AppServer>, String> {
    ...
    // LM Studio accepts this placeholder when authentication is disabled. If
    // a token is configured, it remains in Windows Credential Manager and is
    // exposed only to the local App Server child process.
    command.env(
        "LMSTUDIO_API_KEY",
