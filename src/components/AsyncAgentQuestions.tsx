...
export function AsyncAgentQuestions({ message }: { message: ChatMessage }) {
  ...
  return <section className="agent-questions" aria-label="Agent questions">
    <strong>{saved ? "Answers submitted" : "Your input"}</strong>
    {saved ? <><p>Your answers are submitted. If delivery is queued, its status and Retry control appear above the message box.</p><dl>{questions.map((question) => <div key={question.id}><dt>{question.title}</dt><dd>{saved[question.id]?.join(", ")}</dd></div>)}</dl></> : <>
