...
const LM_STUDIO_GUIDE_URL = "https://lmstudio.ai/docs/developer/rest/quickstart";
...
function ProviderStep({ runtimeStatus, claudeStatus, cursorStatus, account, openRouterReady, lmStudioReady }: {
  ...
  return <div className="onboarding-page providers-page">
    <div className="onboarding-copy">
      ...
      <p>Subscription providers use their official local coding runtime and browser login. OpenRouter uses your API key, while LM Studio runs downloaded models on this PC.</p>
    </div>
    <div className="onboarding-provider-grid">
      <article className="onboarding-provider-card openai">
        ...
        <ol>
          <li><b>1</b><span>Install <strong>Codex CLI for Windows</strong>. OpenKiwi detects the runtime automatically.</span></li>
      ...
      <article className="onboarding-provider-card lmstudio">
        ...
        <ol>
          ...
          <li><b>3</b><span>Choose LM Studio in Models &amp; accounts and select a discovered local model.</span></li>
        </ol>
        <div className="onboarding-card-footer">
          <StatusPill ready={runtimeReady}>{runtimeReady ? "Runtime ready" : "Codex runtime needed"}</StatusPill>
          <StatusPill ready={lmStudioReady}>{lmStudioReady ? "Local model ready" : "Server not connected"}</StatusPill>
        </div>
        <button className="onboarding-link-button" onClick={() => void openUrl(LM_STUDIO_GUIDE_URL)}><ExternalLink size={12} /> LM Studio setup</button>
      </article>
    </div>
    <div className="onboarding-note"><ShieldCheck size={14} /><span>OpenKiwi never asks you to paste a subscription password. ChatGPT, Claude, and Cursor use their official local login; OpenRouter uses your API key; LM Studio stays local.</span></div>
  </div>;
}
