...
describe("destination readiness", () => {
  it("accepts a fully configured destination on every provider", () => {
    for (const entry of [
      ...
      target({ provider: "lmstudio", model: "local/qwen3-coder" }),
  ...
  it("reports the missing sign-in for each provider", () => {
    ...
    expect(childAgentTargetIssue(target({ provider: "lmstudio", model: "local/qwen3-coder" }), { ...EVERYTHING_READY, lmStudioReady: false }))
