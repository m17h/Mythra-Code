...
describe("thread provider resolution", () => {
  ...
  it("preserves provider-owned model ids and enforces only routing constraints", () => {
    expect(modelForProvider("claude", "")).toBe(DEFAULT_CLAUDE_MODEL);
    expect(modelForProvider("claude", "-p")).toBe(DEFAULT_CLAUDE_MODEL);
    expect(modelForProvider("claude", "gpt-5.6-sol")).toBe("gpt-5.6-sol");
    ...
    expect(modelForProvider("openai", "claude-sonnet")).toBe("claude-sonnet");
