...
const legacyAllowDirtyEnv = `OPEN${"KIWI"}_ALLOW_DIRTY`;
const allowDirty = process.argv.includes("--allow-dirty") || process.env.MYTHRA_CODE_ALLOW_DIRTY === "1" || process.env[legacyAllowDirtyEnv] === "1";
...
if (dirtyTree) {
  if (!allowDirty) {
    throw new Error(`Refusing to build a release from a dirty working tree:\n${dirtyTree}\nCommit or stash these changes, or rerun with --allow-dirty (or MYTHRA_CODE_ALLOW_DIRTY=1) to override.`);
  }
  console.warn("############################################################");
...
// Tauri builds and notarizes the app/updater payload only. Mythra Code's DMG is
...
if (process.platform === "darwin") {
  ...
  const appBundle = resolve(macosBundle, "Mythra Code.app");
  const updaterArchive = resolve(macosBundle, "Mythra Code.app.tar.gz");
  ...
  if (appStapled.status !== 0) {
    const notaryZip = resolve(macosBundle, "Mythra Code-notarization.zip");
    ...
    const archive = spawnSync("/usr/bin/tar", ["-czf", updaterArchive, "-C", macosBundle, "Mythra Code.app"], {
      cwd: root,
  ...
  const dmg = resolve(root, `src-tauri/target/release/bundle/dmg/Mythra Code_${version}_${tauriArch}.dmg`);
...
console.log(`Mythra Code ${version} macOS assets are signed for updates and staged for publishing.`);
